...
public class TenantManagementService : ITenantManagementService
{
    ...
    {
        return await _context.PessoasPerfis.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Visitantes.AnyAsync(x => x.TenantId == tenantId)
            ...
            || await _context.Equipes.AnyAsync(x => x.TenantId == tenantId)
            || await _context.HubCasas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Fornecedores.AnyAsync(x => x.TenantId == tenantId)
            || await _context.CategoriasDespesas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.CategoriasReceitas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.ContasBancarias.AnyAsync(x => x.TenantId == tenantId)
            || await _context.CentrosCustos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Projetos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.CategoriasPatrimonio.AnyAsync(x => x.TenantId == tenantId)
            || await _context.PatrimonioItens.AnyAsync(x => x.TenantId == tenantId)
            || await _context.PatrimonioMovimentacoes.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Despesas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Receitas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Cargos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Voluntarios.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Eventos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.EventosRecorrencias.AnyAsync(x => x.TenantId == tenantId)
            || await _context.EventosOcorrencias.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Escalas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.EscalasItens.AnyAsync(x => x.TenantId == tenantId)
            || await _context.EscalasModelos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.EscalasModelosItens.AnyAsync(x => x.TenantId == tenantId)
            || await _context.IndisponibilidadesVoluntarios.AnyAsync(x => x.TenantId == tenantId)
            || await _context.SolicitacoesTrocasEscalas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.DestaquesSite.AnyAsync(x => x.TenantId == tenantId)
            || await _context.ConfiguracoesPortal.AnyAsync(x => x.TenantId == tenantId)
            || await _context.ConfiguracoesCampanhaAniversario.AnyAsync(x => x.TenantId == tenantId)
            || await _context.EnviosCampanhaAniversario.AnyAsync(x => x.TenantId == tenantId)
            || await _context.CategoriasNoticias.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Noticias.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Contatos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.InscricoesEventos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.CategoriasMidias.AnyAsync(x => x.TenantId == tenantId)
            || await _context.GaleriasFotos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.GaleriasFotosItens.AnyAsync(x => x.TenantId == tenantId)
            || await _context.Enquetes.AnyAsync(x => x.TenantId == tenantId)
            || await _context.EnqueteOpcoes.AnyAsync(x => x.TenantId == tenantId)
            || await _context.EnqueteVotos.AnyAsync(x => x.TenantId == tenantId)
            || await _context.CriancasDetalhes.AnyAsync(x => x.TenantId == tenantId)
            || await _context.ResponsaveisCriancas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.KidsCheckins.AnyAsync(x => x.TenantId == tenantId)
            || await _context.KidsNotificacoes.AnyAsync(x => x.TenantId == tenantId)
            || await _context.KidsDeviceTokens.AnyAsync(x => x.TenantId == tenantId)
            || await _context.KidsOcorrencias.AnyAsync(x => x.TenantId == tenantId)
            || await _context.KidsSalas.AnyAsync(x => x.TenantId == tenantId)
            || await _context.KidsTurmas.AnyAsync(x => x.TenantId == tenantId);
    }
