...
export default function CategoriaDespesaForm() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError('Erro ao carregar categoria de despesa');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Erro ao salvar categoria de despesa';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">{isEditing ? 'Editar Categoria de Despesa' : 'Nova Categoria de Despesa'}</h1>
          <p className="text-muted-foreground">{isEditing ? 'Atualize as informações da categoria' : 'Cadastre uma nova categoria de despesa'}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Button type="button" variant="outline" asChild>
            <Link to="/financeiro/categorias-despesas">Cancelar</Link>
          </Button>
        </div>
      </form>
    </div>
