...
export default function CentrosCustosList() {
  ...
  const load = async () => {
    try {
      ...
      setError(t('finance.costCenters.errorLoad'));
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id) => {
    const centro = items.find((c) => c.id === id);
    confirmDialog.show({
      title: t('finance.costCenters.deleteTitle'),
      description: t('finance.costCenters.deleteDescription', { name: centro?.nome || t('finance.costCenters.emptyMessage') }),
      confirmText: t('finance.revenues.delete.confirm'),
      cancelText: t('actions.cancel'),
      variant: 'destructive',
      onConfirm: async () => {
        try {
          await centrosCustosApi.delete(id);
          toast.success(t('finance.costCenters.deleteSuccess'));
          await load();
        } catch (err) {
          toast.error(t('finance.costCenters.deleteError'));
          console.error(err);
          throw err;
        }
      },
  ...
  if (loading) return <LoadingPage text={t('finance.costCenters.loading')} />;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('finance.costCenters.listTitle')} ({total})</CardTitle>
        </CardHeader>
        <CardContent>
          {filtered.length === 0 ? (
