...
export default function EscalasList() {
  ...
  const handleGerarOcorrencias = async () => {
    ...
    try {
      ...
      toast.success(`${res.data?.totalCriadas ?? 0} ocorrência(s) criada(s)`);
      await loadOcorrencias();
    } catch (err) {
      console.error(err);
      toast.error(err.response?.data || 'Erro ao gerar ocorrências');
    }
  };
  ...
  if (loading) return <LoadingPage text="Carregando escalas..." />;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie escalas por ocorrência de evento/culto</p>
        </div>
